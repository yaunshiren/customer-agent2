...
> 第一阶段 4/4 通过；第二阶段 20/20 调用成功但仅 13/20 正确，出现 6 条过度检索和 1 条澄清，
> 未通过挑战门禁。v4 候选不晋级，线上默认继续保持不变。
...
见 `evaluation/reports/m5d-intent-v4-development.json`。第二阶段 20/20 调用成功、0 失败、零自动
重试，但只达到 13/20：10 条应检索全部正确，10 条应直接回答仅 3 条正确；混合商品对比 6/6、
跨生态 4/4、泛品牌评价 2/2，纯第三方具体商品仅 1/8。错误为 6 条 over-retrieval 和 1 条
clarification，under-retrieval 仍为 0；记录 13,041 输入 Token、975 输出 Token，延迟 P50/P95
为 1,131.006/2,740.259 ms。该结果说明仅靠“可能由知识库覆盖”的静态描述，分类器无法可靠知道
当前授权知识库实际覆盖哪些商品；v4 未通过挑战门禁且不晋级。脱敏报告见
`evaluation/reports/m5d-intent-v4-challenge.json`，完整协议与结论见 ADR-0014。
...
- fast 模型按打包的三节点意图树输出三个独立置信分数；KB 节点可用稳定 slug 绑定一个或多个
  知识库，绑定在分类后解析为内部 UUID。最高分至少 `0.75` 且与第二名差值至少
...
授权数必须精确等于当前阶段缺口。报告不保存问题或模型原文。当前缓存已完整包含 24 条，不得重复
执行上述付费命令；v4 因挑战门禁失败，不进入影子或小流量验证。完整限制见 ADR-0014。
