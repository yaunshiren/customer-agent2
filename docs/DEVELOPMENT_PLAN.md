...
切片、v2 候选树和单变量实验保护，并把候选验证拆为缓存复用的 22 条失败集、追加 18 条回归集和
追加 110 条完整集；候选树尚待独立费用授权下从第一阶段开始验证，Rerank 校准不与本实验混合。
...
- M5-D：离线切片确认 22 条 Intent 错误包含 SUPPORT/FEEDBACK/CHAT 的 9/10/3 条，17 条错误
  ...
  `m5-d-v2-candidate`、候选分数记录、确定性阈值重放、树语义哈希 checkpoint 和独立输出保护。
  线上 `m4-c-v1` 保持不变。候选验证已增加严格缓存身份、逐条原子落盘和预登记门禁：先验证
  22 条历史错误，通过后新增 18 条回归保护，前 40 条通过后再补 110 条；完整候选仍以 150 条作
  最终结论，但无效候选通常只支付 22 或 40 次。下一步只授权第一阶段最多 22 次真实调用。
...
| M4 | 意图树结构、阈值和记忆窗口 |
| M5 | M5-D v2 候选树按 22/18/110 分阶段独立授权；是否继续和晋级按 ADR-0013 预登记门槛决定 |
