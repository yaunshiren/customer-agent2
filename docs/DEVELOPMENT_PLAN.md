...
完整集。v2 第一阶段以 18/22 正确但 1 条 under-retrieval 未通过门禁并停止；v3 已离线准备，尚待
新的第一阶段费用授权，Rerank 校准不与本实验混合。
...
- M5-D：离线切片确认 22 条 Intent 错误包含 SUPPORT/FEEDBACK/CHAT 的 9/10/3 条，17 条错误
  ...
  库存、隐含功能建议和竞品边界；下一步需为 v3 第一阶段重新授权最多 22 次真实调用。
...
| M4 | 意图树结构、阈值和记忆窗口 |
| M5 | M5-D 候选树按 22/18/110 分阶段独立授权；v2 已停止，v3 是否继续按 ADR-0013 门槛决定 |
