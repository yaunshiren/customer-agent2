...
完整集。v2 第一阶段以 18/22 正确但 1 条 under-retrieval 未通过门禁并停止；v3 第一阶段已达到
22/22 正确且 under-retrieval 为 0，尚待是否追加 18 条回归保护的独立费用授权。Rerank 校准不与
本实验混合。
...
- M5-D：离线切片确认 22 条 Intent 错误包含 SUPPORT/FEEDBACK/CHAT 的 9/10/3 条，17 条错误
  ...
  11/11 且 under-retrieval 为 0，已通过门禁。下一步只考虑新增 18 条回归保护，尚未授权或执行。
...
| M4 | 意图树结构、阈值和记忆窗口 |
| M5 | M5-D 候选树按 22/18/110 分阶段授权；v3 已过 22 条，是否新增 18 条按 ADR-0013 决定 |
