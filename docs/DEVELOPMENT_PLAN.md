...
尚待是否补齐剩余 110 条的独立费用授权；Rerank 校准不与本实验混合。
...
- M5-D：离线切片确认 22 条 Intent 错误包含 SUPPORT/FEEDBACK/CHAT 的 9/10/3 条，17 条错误
  ...
  SUPPORT/FEEDBACK/CHAT 为 15/15、15/15、10/10，under-retrieval 仍为 0。下一步只考虑补齐
  剩余 110 条，尚未授权或执行。
...
| M4 | 意图树结构、阈值和记忆窗口 |
| M5 | M5-D 候选树按 22/18/110 分阶段授权；v3 已过前 40 条，是否补 110 条按 ADR-0013 决定 |
