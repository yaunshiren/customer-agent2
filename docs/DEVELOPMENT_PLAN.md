...
下一步进入 M5-C 的 150 条完整 Evaluation、失败样本分析和发布加固。
...
- M5-C：已固定原项目附带的 150 条全量集、20 条遗留快速集和 116 篇可检索文档，完成严格
  ...
  `0.6723/0.7683/0.8598`，MRR@10 为 `0.8075`。37 条缺少至少一个 required 文档，其中 2 条
  Top10 完全未命中。150 条 Intent 脱敏运行器、固定映射、分层准确率/混淆矩阵、首错终止和
  精确付费调用确认已实现并通过本地 Fake 测试；最多 132 次付费 Rerank ON、150 次 fast Chat
  Intent 真实运行和最终发布加固待实施。
...
| M4 | 意图树结构、阈值和记忆窗口 |
| M5 | M5-B 的 20 次真实调用已完成；M5-C 数据集已确认，132 次 Rerank 与完整 Intent 费用需另行确认 |
